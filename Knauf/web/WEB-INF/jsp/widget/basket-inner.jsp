    ...
    <table class="products">
        ...
        <c:forEach items="${cart.get('CartItems')}" var="good">
            <tr class="cell-product" rel="item-8122266">
                <td class="delete">
                                        <span class="button cart-delete" good-id="${good.get('ItemId')}"
                                              rel="cart-delete"></span>
                </td>
                <td class="product">
                    <a class="b-picture fancybox-gallery"
                       href="${fn:replace(fn:replace(good.get("Path"),"gif", "jpg"),"small","c200")}"
                       rel="thumbnails"><img
                            src="${fn:replace(fn:replace(good.get("Path"),"gif", "jpg"),"small","c200")}"
                            alt=""></a>
                </td>
                <td class="name">
                ...
                <td class="sum">
                    <input type="number" class="good-quantity" rel="quantity"
                           good-id="${good.get('ItemId')}" min="1" value="${good.get('Quantity')}">
                </td>
                <td class="total">
                    <em class="style-prices">
        ...
        <div class="buttons">
            <c:choose>
                <c:when test="${cart.get('CartSummary').get('FullSum')>amount}">
                    <b style="color: red; font-size: large;">Недостаточно средств. Необходимо <a href="/certificate" style="text-decoration: underline;">активировать
                        сертификат.</a></b>
                </c:when>
                <c:otherwise>
