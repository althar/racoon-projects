...
public class PGSQLDataSource extends DBProcessor
{
    ...
    {
        return getRecords("SELECT * FROM delivery_variant ORDER BY id");
    }
    ...
    {
        Order order = new Order(getRecord("SELECT ord.*,del.name AS delivery_name,del.price AS delivery_price,u.login,u.email,u.name AS user_name FROM order_list ord, user_list u, delivery_variant del WHERE ord.user_id=u.id AND ord.delivery_variant=del.id AND ord.id="+id));
        order.setGoods(getRecords("SELECT * FROM order_good WHERE order_id="+id.toString()));
        return order;
    }

    public void deleteCharacteristics(String brand,String article) throws Exception
    {
        executeNonQuery("DELETE FROM good_characteristic WHERE good_id=(SELECT id FROM good WHERE lower(brand)=lower('"+brand+"') AND lower(article)=lower('"+article+"') LIMIT 1)");
    }
    public void addImage(String brand,String article,String url) throws Exception
    {
        DBRecord rec = getRecord("SELECT id FROM good WHERE lower(brand)=lower('"+brand+"') AND lower(article)=lower('"+article+"')");
        if(rec==null)
        {
            return;
        }
        String good_id = String.valueOf(rec.getLongValue("id"));
        if(getRecord("SELECT id FROM good_image WHERE url='"+url+"'")!=null)
        {
            return;
        }
        executeNonQuery("INSERT INTO good_image (good_id,url) VALUES ("+good_id+",'"+url+"')");
    }
    public void addCharacteristics(String brand,String article,String name,String value) throws Exception
    {
        DBRecord rec = getRecord("SELECT id FROM good WHERE lower(brand)=lower('"+brand+"') AND lower(article)=lower('"+article+"')");
        if(rec==null)
        {
        ...
        String good_id = String.valueOf(rec.getLongValue("id"));
        executeNonQuery("INSERT INTO good_characteristic (good_id,name,value,description,type) VALUES ("+good_id+",'"+name+"','"+value+"','','')");
    }
}