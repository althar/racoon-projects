...
namespace FTwoFramework.Helpers
{
    ...
    public class NumberString
    {
        ...
        {
            string res = RusCurrency.Str(value);
            return res.Replace("рублей 00 копеек", "").Replace("рубл€ 00 копеек", "").Replace("рубль 00 копеек", "");
        }
    }
    ...
    public class StringCalculator
    {
        ...
        public double calculateSimple()
        {
            ...
            for (int i = 0; i < Source.Length; i++)
            {
                ...
                {
                    numbers.Add(Source.Substring(index_start, len));
                    operators.Add(Source[i]);
                    index_start+=len+1;
                    len=0;
                }
                int out_int = 0;
                if(Int32.TryParse(Source[i].ToString(),out out_int)||Source[i]=='*')
                {
                    len++;
                }
            }
            ...
            {
                double operand_1 = Double.Parse(calculateMultipline(numbers[j + 1]));
                if (operators[j] == '+')
                {
                    result += operand_1;
                }
        ...
        private string calculateMultipline(string expression)
        {
            if (expression.Contains("*"))
            {
                string[] operands = expression.Split(new string[] { "*" }, StringSplitOptions.RemoveEmptyEntries);
                return (Double.Parse(operands[0])*Double.Parse(operands[1])).ToString();
            }
            return expression;
        }
    }
