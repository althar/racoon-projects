...
namespace FTwoFramework.Excel
{
    public class FTwoExcelProcessor
    {
        ...
        public int getIndent(int row, int column)
        {
            string addressStart = generateCellAddress(row, column);
            string addressFinish = generateCellAddress(row, column);
            Range workSheet_range = ExcelCurrentSheet.get_Range(addressStart, addressFinish);
            int indent = workSheet_range.Cells.IndentLevel;
            return indent;
        }
